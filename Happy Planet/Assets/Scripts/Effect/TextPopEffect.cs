...
public class TextPopEffect : Effect {
    ...
    public void SetText(string text)
    {
        ...
        _maxSize = _view.fontSize * _bojung;
        _view.enableAutoSizing = false;
        ...
        for (int i = 1; i < chars.Length; i++)
        {
            ...
            _SetText(i, string.Format(" {0} ", chars[i]), new Vector2((i - chars.Length * .5f + 0.5f) * _maxSize, 0));

        }

        _targetText[0] = _view;
        _SetText(0, chars[0].ToString(), new Vector2((-chars.Length * .5f + 0.5f) * _maxSize, 0));
    }
